...
- Test matrix document, plus durable proof rows when the Harness CLI is present.
...
The CLI binary may be absent in a working checkout (`scripts/bin/` can be empty).
When it is absent, do not block product or documentation work and do not invent
durable proof rows. Use [`docs/TEST_MATRIX.md`](TEST_MATRIX.md) as the checked-in
fallback for behavior-to-proof mapping, run the available validation scripts
directly, and report the missing CLI as a process gap in the final response.
Record the durable intake, trace, backlog, and decision rows when the binary is
restored.
...
- A trace has been recorded with `scripts/bin/harness-cli trace`, or the final
  response explicitly notes that the CLI binary is absent.
- Missing harness capabilities were recorded with
  `scripts/bin/harness-cli backlog add`, or documented in the final response
  when the CLI binary is absent.
- The final response says what changed and what was not attempted.

## Current Validation Ladder

Validation scripts now exist in `package.json`; use the suite that matches the
surface touched. The checked-in matrix is in [`docs/TEST_MATRIX.md`](TEST_MATRIX.md)
and the suite details are in [`../TESTING.md`](../TESTING.md).
...
stability and structural proof
  npm run golden
  npm run invariants
  npm run contract

focused subsystem proof
  ...
  npm run export-xlsx
  npm run export-hidden
  npm run suggestions-tests

detector limitation diagnostics
  npm run detection-limitations
```

Agents must not claim a command passes until it has been run in the current
checkout. If a suite is intentionally skipped, say why.
