...
| US-093 | A TD can grow any selected curved Board annotation with optional interior anchor points via a persistent "Add point" tool (consolidated behind a Straight/Curved/Eraser/Text drop-down): insertion lands exactly on the existing path, a fresh anchor's handles default to mirrored (Alt+drag breaks that for one drag, a later plain drag re-mirrors), and selecting an anchor then Backspace removes just it — Backspace with none active still deletes the whole line; an interior anchor also follows a whole-line move and a photo resize (fixed 2026-08-21 — `moveAnnotation`/`scaleAnnotationAbout` predated `ann.points`, found by `/code-review`) | no | no | yes | no | implemented | `npm run board-interaction-check` (sections 4c, 4d, 4e), `npm run board-toolbar-check`, `npm run smoke`, `npm run golden`, `npm run accuracy`, `npm run contract`, `npm run invariants`, `npm run autosave-check` (last six unchanged — no anchor/POM/detection input added, 2-handle default is byte-identical); see [ADR 0053](decisions/0053-curved-lines-grow-extra-anchor-points-on-demand.md) |
