...
| US-086 | After Apply, a POM endpoint is grabbable on the first press, a near-miss never moves the sketch, the photo stays movable via select-then-drag, and a sub-3px press changes nothing | no | no | yes | no | implemented | `npm run board-interaction-check`, `npm run smoke`, `npm run golden`, `npm run accuracy`, `npm run autosave-check`; see [ADR 0050](decisions/0050-lines-are-the-work-photos-are-the-backdrop.md) |
| US-088 | Selecting a line does not move the board on screen, and the canvas backing buffer always matches its CSS box so a line is drawn where the pointer code tests for it | no | no | yes | yes | implemented | `npm run board-interaction-check` (section 0), `npm run smoke`, `npm run golden`, `npm run accuracy`, `npm run preview-check`; see [ADR 0051](decisions/0051-the-board-holds-still-when-the-chrome-moves.md) |
...
| US-093 | A TD can grow any selected curved Board annotation with optional interior anchor points via a persistent "Add point" tool (consolidated behind a Straight/Curved/Eraser/Text drop-down): insertion lands exactly on the existing path, a fresh anchor's handles default to mirrored (Alt+drag breaks that for one drag, a later plain drag re-mirrors), and selecting an anchor then Backspace removes just it — Backspace with none active still deletes the whole line; interior anchors also follow whole-line moves and photo resizes, and measurement/hit/marquee/stitch/label sampling remains accurate as the segment count grows | yes | no | yes | no | implemented | `npm run curve-polyline-tests`, `npm run board-interaction-check` (sections 4c, 4d, 4e), `npm run board-toolbar-check`, `npm run smoke`, `npm run golden`, `npm run accuracy`, `npm run contract`, `npm run invariants`, `npm run autosave-check`; see [ADR 0053](decisions/0053-curved-lines-grow-extra-anchor-points-on-demand.md) |
| US-095 | A TD can create stroke-only Rectangle/Circle/Hexagon Board Graphics, move/resize them, enter Edit Path, edit straight/cubic segments, and Cut Path without creating a POM or separate top-level object; owned graphics follow image move/resize/delete and round-trip through project state | yes | yes | yes | yes | implemented | `npm run board-shape-check` (20 checks), `npm run board-toolbar-check` (54), `npm run board-interaction-check` (250), `npm run keyboard-shortcuts-check` (32), `npm run autosave-check`, `npm run export-hidden`, `npm run preview-check`, `npm run smoke`, `npm run golden`, `npm run invariants`, `npm run contract`; see [ADR 0054](decisions/0054-board-shapes-remain-single-non-measurement-graphics.md) |
