...
//   4. Manual Mode stays removed — mode requests coerce to Auto and the
//      manual drawing-tool shortcuts stay inert (auto-only build).
//   5. Reset Learning leaves meanings intact; Reset Meanings leaves
//      residuals intact.
//   6. Ranking blends distance with recency-weighted usage.
//   7. POM 7 is fixed as cradle height at center bottom cup.
...
async function runMeaningTests(cdp, imagePath) {
  return await evaluate(cdp, `
    (async () => {
      ...
      // --- TEST 1b: POM 7 fixed meaning -------------------------------
      try {
        if (!refDraft) throw new Error('reference draft missing; loadErr=' + loadErr);
        fullReset();

        M.setStyleId('STYLE-POM7');
        M.confirm('7', 'side-height'); // Should not override the fixed rule.
        const resolved = M.resolve('7');
        const ann = makeAnn(7001, '7');
        const evalRes = M.openPopoverForAnn(ann);
        const popoverOpen = M.isPopoverOpen();
        M.cancelPopover();

        const ok =
          resolved && resolved.id === 'cradle-height-bottom-cup'
          && resolved.label === 'Cradle height at center bottom cup'
          && evalRes.status !== 'needsConfirmation'
          && popoverOpen === false;

        record('POM 7 is fixed to cradle height at center bottom cup', ok,
          'resolved=' + (resolved && resolved.id)
          + ' status=' + (evalRes && evalRes.status)
          + ' popoverOpen=' + popoverOpen,
          { resolved, evalRes, popoverOpen });
      } catch (e) {
        record('POM 7 is fixed to cradle height at center bottom cup', false, String(e && e.message || e));
      }
      ...
      // --- TEST 4: Manual Mode unlock — mode switch + tool shortcuts ------
      // Manual Mode is re-enabled (Manual Mode re-enable slice):
      // setAppMode('manual') switches for real and unlocks the drawing-tool
      // shortcuts, while Auto Mode still blocks them outright so project
      // annotations stay locked during review. Guard both halves of the
      // new contract, then return to Auto for the remaining tests.
      try {
        fullReset();
        M.setStyleId('STYLE-KB');
        M.setAppMode('manual');
        const modeAfterManualRequest = debug.getState().appMode;
        ...
        fire('0', 'Digit0');
        const manualToolAfter0 = M.getCanvasTool();
        fire('b', 'KeyB');
        const manualToolAfterB = M.getCanvasTool();
        fire('s', 'KeyS');
        const manualToolAfterS = M.getCanvasTool();

        M.setAppMode('auto');
        const modeAfterAutoRequest = debug.getState().appMode;
        fire('0', 'Digit0');
        const autoToolAfter0 = M.getCanvasTool();
        fire('b', 'KeyB');
        const autoToolAfterB = M.getCanvasTool();
        fire('c', 'KeyC');
        const autoToolAfterC = M.getCanvasTool();

        const ok =
          modeAfterManualRequest === 'manual'
          && toolBefore === 'select'
          && manualToolAfter0 === 'straight'
          && manualToolAfterB === 'curved'
          && manualToolAfterS === 'select'
          && modeAfterAutoRequest === 'auto'
          && autoToolAfter0 === 'select'
          && autoToolAfterB === 'select'
          && autoToolAfterC === 'select';

        record('Manual Mode unlock: mode switch + tool shortcuts', ok,
          'manualMode=' + modeAfterManualRequest + ' before=' + toolBefore
          + ' m0=' + manualToolAfter0 + ' mB=' + manualToolAfterB
          + ' mS=' + manualToolAfterS
          + ' autoMode=' + modeAfterAutoRequest
          + ' a0=' + autoToolAfter0 + ' aB=' + autoToolAfterB + ' aC=' + autoToolAfterC,
          {
            modeAfterManualRequest, toolBefore,
            manualToolAfter0, manualToolAfterB, manualToolAfterS,
            modeAfterAutoRequest, autoToolAfter0, autoToolAfterB, autoToolAfterC,
          });
      } catch (e) {
        record('Manual Mode unlock: mode switch + tool shortcuts', false, String(e && e.message || e));
      }
      ...
      // --- TEST 7: Out-of-template POM (17) surfaces, not silently dropped
      // Before the parser fix, a label outside 1–16 parsed to null and the
      // sample was skipped with no prompt. It must now route to the picker.
      try {
        if (!refDraft) throw new Error('reference draft missing; loadErr=' + loadErr);
        fullReset();
        M.setStyleId('STYLE-UNK');
        const evalRes = L.evaluateSample(makeAnn('m17a', '17'));
        const ok = evalRes.status === 'needsConfirmation' && evalRes.pom === '17';
        record('Out-of-template POM 17 surfaces for confirmation (not skipped)', ok,
          'status=' + evalRes.status + ' pom=' + evalRes.pom,
          { evalRes });
      } catch (e) {
        record('Out-of-template POM 17 surfaces for confirmation (not skipped)', false, String(e && e.message || e));
      }

      // --- TEST 8: Confirming a custom meaning for POM 17 persists & resolves
      try {
        if (!refDraft) throw new Error('reference draft missing; loadErr=' + loadErr);
        fullReset();
        M.setStyleId('STYLE-UNK');
        const commit = M.commitCustom(makeAnn('m17b', '17'), 'Back center lower height');
        const resolved = M.resolve('17');
        const rows = (M.summarize().currentRows) || [];
        const row17 = rows.find(r => String(r.pom) === '17');
        const ok =
          commit.applied === true
          && resolved && /back center lower height/i.test(resolved.label)
          && !!row17;
        record('POM 17 custom meaning is confirmed, persists, and shows in panel', ok,
          'applied=' + commit.applied + ' resolved=' + (resolved && resolved.label) + ' inPanel=' + !!row17,
          { commit, resolved, row17 });
      } catch (e) {
        record('POM 17 custom meaning is confirmed, persists, and shows in panel', false, String(e && e.message || e));
      }

      // --- TEST 9: Numbers above the ceiling are ignored; POM 16 still works
      // Guards against learning garbage from incidental numbers in labels
      // while confirming in-template POMs are unaffected by the fix.
      try {
        if (!refDraft) throw new Error('reference draft missing; loadErr=' + loadErr);
        fullReset();
        M.setStyleId('STYLE-CEIL');
        const big  = L.evaluateSample(makeAnn('mbig',  '200'));     // 3 digits → no match → skipped
        const huge = L.evaluateSample(makeAnn('mhuge', 'meas 99')); // 99 > POM_LABEL_MAX → skipped
        const p16  = M.openPopoverForAnn(makeAnn('m16', '16'));     // in-template → needsConfirmation
        M.cancelPopover();
        const ok =
          big.status === 'skipped'
          && huge.status === 'skipped'
          && p16.status === 'needsConfirmation' && p16.pom === '16';
        record('Numbers above the POM ceiling are ignored; POM 16 still surfaces', ok,
          'big=' + big.status + ' huge=' + huge.status + ' p16=' + p16.status + '/' + p16.pom,
          { big, huge, p16 });
      } catch (e) {
        record('Numbers above the POM ceiling are ignored; POM 16 still surfaces', false, String(e && e.message || e));
      }
